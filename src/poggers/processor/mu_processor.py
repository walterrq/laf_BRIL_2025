...
class MuProcessorExtension(ABC):
    ...
    @abstractmethod
    def build_dataframe(self, buffer: List[Tuple[Any, ...]]) -> pd.DataFrame:
        pass
...
class MuProcessor(HD5Processor):
    ...
    def process_iteration(self, ctx: IterationContext):
        ...
        if c_handle.nrows == 0 or c_handle.nrows == 0:
        ...
        nbx, bxmask = self._get_nbx_bxmask(b_handle, ls_query)
    ...
    @staticmethod
    def _get_nbx_bxmask(beam: tb.Table, ls_query: str) -> Tuple[int, np.ndarray]:
        i = -1
        ...
        if i > -1:
            ...
            # we reach this if the table does have a single row
