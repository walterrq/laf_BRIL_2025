...
class HFOCMuExtension(MuProcessorExtension):
    def process_batch(self, batch: pd.DataFrame, nbx: int, bxmask: np.ndarray) -> pd.DataFrame:
        data = np.stack(batch["bxraw"])
        batch["avgraw"] = np.sum(data * bxmask, axis=1)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", type=Path, help="Path to output.", default=Path("poggers_hfoc_example"))
